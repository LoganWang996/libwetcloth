...
void SceneStepper::allocateLagrangianVectors(const TwoDScene& scene,
                                             VectorXs& vec) {
...
void SceneStepper::buildLocalGlobalMapping(
  ...
  allocateNodeVectors(scene, node_global_indices_x, node_global_indices_y,
                      node_global_indices_z);
  ...
  buckets.for_each_bucket([&](int bucket_idx) {
    ...
    for (int i = 0; i < num_nodes_x; ++i) {
      if (node_global_indices_x[bucket_idx][i] >= 0) {
        ...
        dof_ijk[global_idx] =
            Vector3i(handle(0) * bucket_num_cell * 3 + local_handle(0),
    ...
    for (int i = 0; i < num_nodes_y; ++i) {
      if (node_global_indices_y[bucket_idx][i] >= 0) {
        ...
        dof_ijk[global_idx] = Vector3i(
            handle(0) * bucket_num_cell * 3 + bucket_num_cell + local_handle(0),
            handle(1) * bucket_num_cell + local_handle(1),
            handle(2) * bucket_num_cell + local_handle(2));
      }
    }
    ...
    for (int i = 0; i < num_nodes_z; ++i) {
      if (node_global_indices_z[bucket_idx][i] >= 0) {
        ...
        dof_ijk[global_idx] =
            Vector3i(handle(0) * bucket_num_cell * 3 + bucket_num_cell * 2 +
                         local_handle(0),
                     handle(1) * bucket_num_cell + local_handle(1),
                     handle(2) * bucket_num_cell + local_handle(2));
      }
...
void SceneStepper::buildNodeToSoftParticlesMat(
    ...
    const std::vector<Vector3i>& effective_node_indices, TripletXs& tri_W,
    SparseXs& W) const {
  const int num_soft_elasto = scene.getNumSoftElastoParticles();
    ...
    for (int i = 0; i < 27; ++i) {
      if (!scene.isBucketActivated(indices_x(i, 0)) || weights(i, 0) == 0.0) {
        ...
        const int global_idx =
            node_global_indices_x[indices_x(i, 0)][indices_x(i, 1)];

        if (global_idx == -1) {
          tri_W[(pidx * 3 + 0) * 27 + i] = Triplets(0, 0, 0.0);
        } else {
          tri_W[(pidx * 3 + 0) * 27 + i] =
              Triplets(pidx * 4 + 0, global_idx, weights(i, 0));
        }
      }
      ...
      if (!scene.isBucketActivated(indices_y(i, 0)) || weights(i, 1) == 0.0) {
        ...
        const int global_idx =
            node_global_indices_y[indices_y(i, 0)][indices_y(i, 1)];

        if (global_idx == -1) {
          tri_W[(pidx * 3 + 1) * 27 + i] = Triplets(0, 0, 0.0);
        } else {
          tri_W[(pidx * 3 + 1) * 27 + i] =
              Triplets(pidx * 4 + 1, global_idx, weights(i, 1));
        }
      }
      ...
      if (!scene.isBucketActivated(indices_z(i, 0)) || weights(i, 2) == 0.0) {
        ...
        const int global_idx =
            node_global_indices_z[indices_z(i, 0)][indices_z(i, 1)];

        if (global_idx == -1) {
          tri_W[(pidx * 3 + 2) * 27 + i] = Triplets(0, 0, 0.0);
        } else {
          tri_W[(pidx * 3 + 2) * 27 + i] =
              Triplets(pidx * 4 + 2, global_idx, weights(i, 2));
        }
      }
    }
  });
  ...
  tri_W.erase(
      std::remove_if(tri_W.begin(), tri_W.end(),
...
void SceneStepper::mapSoftParticlesToNode(const TwoDScene& scene,
...
void SceneStepper::mapSoftParticlesToNodeSqr(const TwoDScene& scene,
                                             std::vector<VectorXs>& node_vec_x,
                                             std::vector<VectorXs>& node_vec_y,
                                             std::vector<VectorXs>& node_vec_z,
                                             const VectorXs& part_vec) const {
...
void SceneStepper::mapGaussToNode(const TwoDScene& scene,
                                  std::vector<VectorXs>& node_vec_x,
                                  std::vector<VectorXs>& node_vec_y,
                                  std::vector<VectorXs>& node_vec_z,
...
void SceneStepper::allocateCenterNodeVectors(
    const TwoDScene& scene, std::vector<VectorXi>& node_vec_p) const {
...
void SceneStepper::allocateCenterNodeVectors(
    const TwoDScene& scene, std::vector<VectorXs>& node_vec_p) const {
  const int num_buckets = scene.getNumBuckets();
...
void SceneStepper::allocateNodeVectors(
    const TwoDScene& scene, std::vector<VectorXs>& node_vec_x,
    std::vector<VectorXs>& node_vec_y,
...
void SceneStepper::allocateNodeVectors(
    const TwoDScene& scene, std::vector<VectorXi>& node_vec_x,
...
scalar SceneStepper::dotNodeVectors(const std::vector<VectorXs>& node_vec_ax,
  ...
  return dotNodeVectors(node_vec_ax, node_vec_ay, node_vec_az, node_vec_bx,
                        node_vec_by, node_vec_bz) +
