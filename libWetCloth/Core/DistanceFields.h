...
struct DistanceField {
  DistanceField(DISTANCE_FIELD_TYPE type_, DISTANCE_FIELD_USAGE usage_,
  ...
  virtual bool check_durations(const scalar& cur_time, const scalar& cur_vol,
                               Vector3s& shooting_vel) = 0;
  virtual void resample_mesh(const scalar& dx, VectorXs& result,
                             VectorXs& normals) = 0;
  virtual void resample_internal(const std::shared_ptr<TwoDScene>& parent,
  ...
  virtual void render(
      const std::function<void(
          const std::vector<Vector3s>&, const std::vector<Vector3i>&,
...
struct DistanceFieldOperator : public DistanceField {
  DistanceFieldOperator(DISTANCE_FIELD_TYPE type_, DISTANCE_FIELD_USAGE usage_,
                        int group_, int params_index_, bool sampled_);
...
struct DistanceFieldObject : public DistanceField {
  DistanceFieldObject(const Vector3s& center_, const VectorXs& parameter_,
                      DISTANCE_FIELD_TYPE type_, DISTANCE_FIELD_USAGE usage_,
                      bool inside, const Vector3s& raxis, const scalar& rangle,
                      int group_, int params_index_, bool sampled_,
