...
enum ParticleClassifier { PC_NONE, PC_S, PC_s, PC_o, PC_l, PC_L };
...
class TwoDScene : public std::enable_shared_from_this<TwoDScene> {
 ...
 public:
  ...
  void setFluidMass(int particle, const scalar& mass,
  ...
  const std::vector<std::pair<int, int> >& getNodeParticlePairsX(
      int bucket_idx, int pidx) const;

  const std::vector<std::pair<int, int> >& getNodeGaussPairsX(int bucket_idx,
                                                              int pidx) const;

  const std::vector<std::pair<int, int> >& getNodeParticlePairsY(
      int bucket_idx, int pidx) const;

  const std::vector<std::pair<int, int> >& getNodeGaussPairsY(int bucket_idx,
                                                              int pidx) const;

  const std::vector<std::pair<int, int> >& getNodeParticlePairsZ(
      int bucket_idx, int pidx) const;

  const std::vector<std::pair<int, int> >& getNodeGaussPairsZ(int bucket_idx,
                                                              int pidx) const;
 ...
 private:
  ...
  MatrixXs
      m_grad_gauss;  // 3m x 3 (or 2) vectors for computing gradient on Gauss
  ...
  std::vector<Vector3i> m_face_inv_mapping;  // location of the face itself in
                                             // particle_to_face array.
  std::vector<Vector2i> m_edge_inv_mapping;  // location of the edge itself in
                                             // particle_to_face array.
  ...
  std::vector<VectorXi>
      m_node_pressure_neighbors;  // bucket id -> 6x2 neighbors of pressure
                                  // nodes
  std::vector<VectorXi> m_node_pp_neighbors;  // bucket id -> 18x2 pressure
  ...
  std::vector<VectorXi> m_node_index_solid_phi_x;  // bucket id -> 4x2 neighbors
                                                   // to the solid phi nodes
  std::vector<VectorXi> m_node_index_solid_phi_y;  // bucket id -> 4x2 neighbors
                                                   // to the solid phi nodes
  std::vector<VectorXi> m_node_index_solid_phi_z;  // bucket id -> 4x2 neighbors
                                                   // to the solid phi nodes

  std::vector<VectorXi> m_node_index_edge_x;  // bucket id -> 4x2 neighbors to
                                              // the edge nodes (ey, ez)
  std::vector<VectorXi> m_node_index_edge_y;  // bucket id -> 4x2 neighbors to
                                              // the edge nodes (ex, ez)
  std::vector<VectorXi> m_node_index_edge_z;  // bucket id -> 4x2 neighbors to
                                              // the edge nodes (ex, ey)
  ...
  std::vector<VectorXs>
      m_node_pore_pressure_p;  // bucket id -> nodes saturation
  ...
  std::vector<VectorXs>
      m_node_solid_weight_x;  // bucket id -> node solid weight
  ...
  std::vector<VectorXs>
      m_node_vel_saved_fluid_x;  // bucket id -> nodes velocity
